...
class SCCMScripts(WMIFunction):
    ...
    def get(self, guid: str):
        return self.iWbemServices.GetObject(f"SMS_Scripts.ScriptGuid='{guid}'")[0]
    ...
    def get_execution_summary_lazy(self, scriptGuid: str = None):
        whereCond = None
        if scriptGuid:
            whereCond = f"ScriptGuid='{scriptGuid}'"

        return self.get_class_instances("SMS_ScriptsExecutionSummary", properties = None, where = whereCond)

    def add(self, scriptName: str, content: str, timeout: int = 60):
        script, _ = self.iWbemServices.GetObject("SMS_Scripts")
        guid = uuid.uuid4()
    ...
    def remove(self, scriptGuid: str):
        return self.checkiWbemResponse(
            ...
            self.iWbemServices.DeleteInstanceAsync(f"SMS_Scripts.ScriptGuid='{scriptGuid}'")
        )
